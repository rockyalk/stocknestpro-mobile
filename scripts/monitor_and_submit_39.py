...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 monitor_and_submit_39.py <build_id>")
        sys.exit(1)
    ...
    while True:
        ...
        try:
            ...
            if status == "finished":
                print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] EAS Build 39 completed successfully!")
                break
            elif status == "failed":
                print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] EAS Build 39 failed!")
                sys.exit(1)
            elif status == "canceled":
                print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] EAS Build 39 was canceled!")
                sys.exit(1)
    ...
    report_path = "/home/ubuntu/stocknestpro-mobile/submission_report_v39_success.md"
    with open(report_path, "w") as f:
        f.write(f"# EAS Build 39 Submission Success Report\n\n")
        f.write(f"- **Build ID:** `{build_id}`\n")
        f.write(f"- **Build Number:** `39`\n")
        f.write(f"- **Version Code:** `39`\n")
